...
old_make_clean_names <- function(string) {
  ...
  old_names <- string
  new_names <- old_names %>%
    gsub(pattern = "'", replacement = "") %>% # remove quotation marks
    gsub(pattern = "\"", replacement = "") %>% # remove quotation marks
    gsub(pattern = "%", replacement = "percent") %>%
    gsub(pattern = "^[ ]+", replacement = "") %>%
    make.names() %>%
    gsub(pattern = "[.]+", replacement = "_") %>% # convert 1+ periods to single _
    gsub(pattern = "[_]+", replacement = "_") %>% # fix rare cases of multiple consecutive underscores
    tolower() %>%
    gsub(pattern = "_$", replacement = "") # remove string-final underscores
