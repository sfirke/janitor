...
#'
#' A `tabyl` can be the result of a call to `janitor::tabyl()`, in which case these attributes are added automatically.  This function adds `tabyl` class attributes to a data.frame that isn't the result of a call to `tabyl` but meets the requirements of a two-way tabyl:
...
#' @param axes is this a two_way tabyl or a one_way tabyl?  If this function is being called by a user, this should probably be "2".  One-way tabyls are created by `tabyl` but are a special case.
#' @param row_var_name (optional) the name of the variable in the row dimension; used by `adorn_title()`.
#' @param col_var_name (optional) the name of the variable in the column dimension; used by `adorn_title()`.
...
#' @return Returns the same data.frame, but without the `tabyl` class and attributes.
#' @export
#' @examples
#'
#' mtcars %>%
