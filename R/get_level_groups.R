...
get_level_groups <- function(vec, n, num_levels_in_var){
  ...
  if(!is.na(mid_lvls) & nchar(mid_lvls) > 30){ mid_lvls <- paste0("<<< Middle Group (", num_levels_in_var - 2 * n," categories) >>>")}
  if(nchar(top_n_lvls) > 30){ top_n_lvls <- paste0(substr(top_n_lvls, 1, 27), "...")}
  if(nchar(bot_n_lvls) > 30){ bot_n_lvls <- paste0(substr(bot_n_lvls, 1, 27), "...")}
