...
test_that("outputs are correct", {
  expect_equal(coalesce(dat$a, dat$b, dat$c), c(1, 2, 3))
  expect_equal(coalesce(dat$d1, dat$d2), c(as.Date("1999-01-01"), as.Date("1999-02-02"), as.Date("2016-03-03")))
  expect_equal(suppressWarnings(coalesce(dat$a, dat$x)), # mismatched types, supposed to throw warning
               c("1", "med", "lo"))
  expect_equal(suppressWarnings(coalesce(dat$a, dat$d1)),
...
test_that("factors are handled correctly", {
  x <- factor(c("a", "b", NA))
  y <- factor(c("c", "d", "e"))
  expect_equal(suppressWarnings(coalesce(x, y)), c("a", "b", "e")) # factors should throw warnings
})
...
test_that("POSIXct outputs are correct and retain proper class", {
  expect_equal(coalesce(dat$p1, dat$p2),
...
microbenchmark(
  coalesce = coalesce(dat$p1, dat$p2),
  janitor = use_first_valid_of(dat$p1, dat$p2)
)