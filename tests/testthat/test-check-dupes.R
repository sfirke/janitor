...
test_that("Simple duplicates are found", {
  expect_false(check_dupes(dupe_df, level="logical"))
...
test_that("Handes no duplicates", {
  expect_true(check_dupes(id_df, level="logical"))
...
test_that("NAs count as duplicates.", {
  expect_false(check_dupes(NA_df, level="logical"))
...
test_that("works on variables with irregular names", {
  ...
  expect_false(check_dupes(test_df, `bad name`, level="logical"))
